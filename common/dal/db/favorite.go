...
func GetFavoriteRelation(ctx context.Context, uid int64, vid int64) (*Video, error) {
	user := new(User)
	if err := dal.DB.WithContext(ctx).First(user, uid).Error; err != nil {
		return nil, err
	}
	...
	if err := dal.DB.WithContext(ctx).Model(&user).Association("FavoriteVideos").Find(&video, vid); err != nil {
		return nil, err
	}
	return video, nil
}
...
func FavoriteList(ctx context.Context, uid int64) ([]Video, error) {
	...
	if err := dal.DB.WithContext(ctx).Model(&user).Association("FavoriteVideos").Find(&videos); err != nil {
		return nil, err
	}
	return videos, nil
}
